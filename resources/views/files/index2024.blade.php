...
<a href="#" onclick="confirmLogout(event)"><button class="flex items-center space-x-2 bg-sky-300 hover:bg-sky-400 text-sky-700 rounded-full px-4 py-2 shadow-md text-sm font-medium" type="button">
  ...
  <main class="px-6">
   ...
   <form action="{{ route('files.restoreAll2024') }}" method="POST">
        @csrf
...
<form action="{{ route('files.store2024') }}" method="POST" enctype="multipart/form-data">

@csrf
  <label for="year">Year:</label>
  <input type="text" id="year" name="year" min="2000" max="{{ date('Y') }}" placeholder="Enter year" value="2024" readonly style="background-color:#eee;">
  ...
  <button class="btn btn-sm btn-primary" type="submit">Upload</button>
    ...
    <form action="{{ route('files.index2024') }}" method="GET" class="mb-3 d-flex" role="search">
                              ...
                              @forelse ($files as $file)
                                    <tr>
                                        ...
                                        <a href="{{ route('files.show2024', $file->id) }}" target="_blank" class="btn btn-sm btn-secondary">Preview</a>
                                         <form id="delete-form-{{ $file->id }}" action="{{route('files.destroy2024', $file->id)}}" method="POST" style="display:inline">
                               @csrf
                               @method('DELETE')
