  ...
  <main class="px-6">
   <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    ...
    Kembali
      ...
      <form action="{{ route('files.restoreAll2025') }}" method="POST">
        @csrf
...
@csrf
  ...
  <button class="btn btn-sm btn-primary" type="submit">Upload</button>
    ...
    <form action="{{ route('files.index2025') }}" method="GET" class="mb-3 d-flex" role="search">
                            ...
                            <tbody>
                              @forelse ($files as $file)
                                    <tr>
                                        ...
                                        <a href="{{ route('files.download2025', $file->id) }}" class="btn btn-sm btn-primary">Download</a>
                                        <a href="{{ route('files.show2025', $file->id) }}" target="_blank" class="btn btn-sm btn-secondary">Preview</a>
                                         <form id="delete-form-{{ $file->id }}" action="{{route('files.destroy2025', $file->id)}}" method="POST" style="display:inline">
                               @csrf
                               @method('DELETE')
